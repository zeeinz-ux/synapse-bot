document.addEventListener("DOMContentLoaded", () => {
  ...
  if (toggleEnabled && statusBanner && statusText) {
    toggleEnabled.addEventListener("change", function () {
      if (this.checked) {
        ...
        statusText.innerHTML = "Modul Leave sedang <strong>aktif</strong>.";
      } else {
        statusBanner.className = "status-banner inactive";
        statusText.innerHTML = "Modul Leave sedang <strong>nonaktif</strong>.";
      }
    });
  }
  ...
  if (embedTitleInput && embedPreviewTitle) {
    embedTitleInput.addEventListener("input", function () {
      embedPreviewTitle.textContent = this.value.trim() || "👋 Selamat Tinggal!";
    });
  }

  if (bannerTextInput && koyaBannerTitle) {
    bannerTextInput.addEventListener("input", function () {
      koyaBannerTitle.textContent = (this.value.trim() || "GOODBYE").toUpperCase();
    });
  }
  ...
  if (leaveForm) {
    leaveForm.addEventListener("submit", async function (e) {
      ...
      try {
        const res = await fetch("/dashboard/" + guildId + "/welcome/leave/save", { method: "POST", body: formData });
