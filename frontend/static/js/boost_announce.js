document.addEventListener("DOMContentLoaded", () => {
  ...
  if (toggleEnabled && statusBanner && statusText) {
    toggleEnabled.addEventListener("change", function () {
      if (this.checked) {
        ...
        statusText.innerHTML = "Modul Boost sedang <strong>aktif</strong>.";
      } else {
        statusBanner.className = "status-banner inactive";
        statusText.innerHTML = "Modul Boost sedang <strong>nonaktif</strong>.";
      }
    });
  }
  ...
  if (embedTitleInput && embedPreviewTitle) {
    embedTitleInput.addEventListener("input", function () {
      embedPreviewTitle.textContent = this.value.trim() || "💎 Server Boost!";
    });
  }

  if (bannerTextInput && koyaBannerTitle) {
    bannerTextInput.addEventListener("input", function () {
      koyaBannerTitle.textContent = (this.value.trim() || "BOOSTER").toUpperCase();
    });
  }
  ...
  if (boostForm) {
    boostForm.addEventListener("submit", async function (e) {
      ...
      try {
        const res = await fetch("/dashboard/" + guildId + "/welcome/boost/save", { method: "POST", body: formData });
