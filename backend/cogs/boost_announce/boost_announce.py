...
class BoostAnnounceCog(commands.Cog, name="BoostAnnounce"):
    ...
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self._boost_locks = {}
        print("[BOOST-ANNOUNCE] ✅ BoostAnnounceCog loaded")

    def _get_lock(self, guild_id: str, user_id: str) -> asyncio.Lock:
        key = f"{guild_id}:{user_id}"
        if key not in self._boost_locks:
            self._boost_locks[key] = asyncio.Lock()
        return self._boost_locks[key]
    ...
    async def get_boost_config(self, guild_id: str) -> dict | None:
        ...
        try:
            ...
            if not doc.exists:
                return None
            cfg = doc.to_dict().get("boost_announce", {})
            if not cfg.get("enabled", False):
                return None
            return cfg
        except Exception as e:
            print(f"[BOOST-ANNOUNCE] ❌ Firestore error: {e}")
            return None

    async def _send_boost(self, member: discord.Member):
        guild_id = str(member.guild.id)
        user_id = str(member.id)

        lock = self._get_lock(guild_id, user_id)
        async with lock:
            print(f"[BOOST-ANNOUNCE] 💎 {member.name} boosted {member.guild.name}")

            cfg = await self.get_boost_config(guild_id)
            if cfg is None:
                return
            ...
            raw_text = cfg.get("message_text", "{user} telah melakukan boost pada {server}! 💎")
            ...
            if not cfg.get("embed_color"):
                cfg["embed_color"] = "#9B59B6"
            if not cfg.get("banner_text"):
                cfg["banner_text"] = "BOOSTER"
            if not cfg.get("embed_title"):
                cfg["embed_title"] = "💎 Server Boost!"
            ...
            try:
                ...
                print(f"[BOOST-ANNOUNCE] ✅ Sent to #{channel.name} in {member.guild.name}")

            except Exception as e:
                print(f"[BOOST-ANNOUNCE] ❌ Error: {type(e).__name__}: {e}")

    @commands.Cog.listener()
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        if before.premium_since is None and after.premium_since is not None:
            print(f"[BOOST-ANNOUNCE] 📤 Boost: {after.name} @ {after.guild.name}")
            ...
            async with aiohttp.ClientSession(headers=headers, timeout=timeout) as session:
                async with session.get(url, allow_redirects=True) as resp:
                    if resp.status == 200:
                        ...
                        print(f"[BOOST-ANNOUNCE] ⚠️ HTTP {resp.status}")
                        return None
        except Exception as e:
            print(f"[BOOST-ANNOUNCE] ⚠️ Download error: {type(e).__name__}: {e}")
            return None
    ...
    async def _generate_banner_image(self, member: discord.Member, cfg: dict) -> discord.File | None:
        try:
            ...
            welcome_text = " ".join(cfg.get("banner_text", "BOOSTER").upper())
            ...
            output.seek(0)
            return discord.File(output, filename=f"boost_{member.id}.png")

        except Exception as e:
            print(f"[BOOST-ANNOUNCE] ❌ Banner error: {type(e).__name__}: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    async def _send_banner(self, channel, member, cfg, parsed_text):
        ...
        if banner_file:
            ...
            print(f"[BOOST-ANNOUNCE] 📤 Banner + text sent to #{channel.name}")
        else:
            await self._send_embed(channel, member, cfg, parsed_text)

    async def _send_embed(self, channel, member, cfg, parsed_text):
        color_hex = cfg.get("embed_color", "#9B59B6").lstrip("#")
        try:
            embed_color = discord.Color(int(color_hex, 16))
        except:
            embed_color = discord.Color(0x9B59B6)

        raw_title = cfg.get("embed_title", "💎 Server Boost!")
...
async def setup(bot: commands.Bot):
    await bot.add_cog(BoostAnnounceCog(bot))
