...
class LeaveSettingsCog(commands.Cog, name="LeaveSettings"):
            ...
            try:
                ...
                if os.path.exists(fpath):
                    with open(fpath, "rb") as f:
                        data = f.read()
                    print(f"[LEAVE] ✅ Read gallery file: {os.path.basename(url)} ({len(data)} bytes)")
                    return data
                print(f"[LEAVE] ⚠️ Gallery file not found: {fpath}")
                return None
            except Exception as e:
                print(f"[LEAVE] ⚠️ Gallery read error: {type(e).__name__}: {e}")
                return None
        try:
            headers = {
    ...
    async def _generate_banner_image(self, member: discord.Member, cfg: dict) -> discord.File | None:
        try:
            ...
            welcome_text = " ".join(cfg.get("banner_text", "GOODBYE").upper())
